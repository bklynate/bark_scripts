...
def chunk_text(text, max_chars=1000):
    ...
    for word in words:
        ...
        # If the future chunk would exceed the max_chars, save the current chunk and start a new one
        if len(future_chunk) > max_chars:
...
"Palantir has caught the attention of many due to its innovative tech approach. But what about its financial health? Over the last four years, there's been a steady revenue growth, with a 47% increase from 2019 to 2020. However, it wasn’t all positive.
...
"Stay tuned for more stock analysis and remember, invest wisely!"
"""
...
for i, chunk in enumerate(text_chunks):
    print(f'Chunk {i}: {len(chunk)} characters')  # Print characters per chunk
